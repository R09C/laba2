""
    ...
    создаётся специальный обработчик сигнала SIGHUP.Он следит за системными изменениями, например, перезагрузкой или изменением конфигурации.

                                                                                                   Чтобы всё работало,
    нужно зарегистрировать обработчик с помощью структуры sigaction.Сигнал блокируется на время обработки.

        Создание серверного сокета
            Создаём серверный сокет с помощью вызова
            socket()
                .Он настроен на использование протокола IPv4 и TCP.Затем привязываем сокет к определённому адресу и порту с помощью вызова bind()
                .Порт 8080 — это стандартный HTTP
        - порт,
    который мы используем для нашего простого веб - сервера.С помощью функции listen() сервер начинает прослушивать входящие соединения.

                                                    Основной цикл сервера
                                                    В основном цикле работы сервера используем функцию select(),
    чтобы сервер мог обрабатывать несколько соединений одновременно.Когда сервер получает новое подключение, оно принимается с помощью функции accept().Новый сокет добавляется в список наблюдаемых файловых дескрипторов.Когда данные приходят от клиента, они считываются с помощью функции recv().Если данные получены, сервер отправляет HTTP - ответ с сообщением «Hello, world !».Если соединение закрывается, сокет удаляется из списка и закрывается.

                                                                                                                                                                                                                                                                                                                                                                                                                      Сигнал SIGHUP Если получен сигнал SIGHUP,
    сервер обрабатывает его(устанавливается флаг wasSigHup), выводит сообщение и сбрасывает флаг.EOF ""
                                                                                                     "
...
int main()
{
    ...
    while (1)
    {
        ...
        // Восстанавливаем блокированные сигналы перед pselect
        int ret = pselect(max_fd + 1, &read_fds, NULL, NULL, NULL, &origSigMask); // Используем pselect для мультиплексирования

        if (ret == -1)
        {
            if (errno == EINTR)
                continue;      // Если pselect был прерван сигналом, продолжаем
